...
export default function InputDetails() {
    ...
    const handleNext = () => {
        ...
        if (activeStep === steps.length - 1 && businessDetails && businessDetails.value[0].type == 'pharmacy') {
            handleToggle()
            let body = {
            ...
            API.post('pharmacy/distanceToBStation', body).then((distanceToBusStationResult)=> {
                ...
                let pharmacyObj = {
                    ...
                    "distanceToBus": busStationDistance,
                    "OpenHours": businessDetails.value[0].serviceDetails.open ? 1 : 0,
                    "deliver": businessDetails.value[0].serviceDetails.deliver ? 1 : 0,
                    "WhlChairEntrance": businessDetails.value[0].serviceDetails.whlentrence ? 1 : 0,
                    "WhlChairPark": businessDetails.value[0].serviceDetails.whlpark ? 1 : 0,
                    "cashPay": businessDetails.value[0].serviceDetails.cash ? 1 : 0,
